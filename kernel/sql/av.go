...
func fillAttributeViewAutoGeneratedValues(attrView *av.AttributeView, collection av.Collection, ials map[string]map[string]string, depth *int, cachedAttrViews map[string]*av.AttributeView) {
					...
					if nil != blockValues {
						...
						for _, blockID := range value.Relation.BlockIDs {
							if val := blocks[blockID]; nil != val {
								// 确保 Subtype 字段有值
								val.Block.Subtype = "d" // 默认为动态锚文本
								// 检查是否有静态锚文本
								attrs := GetBlockAttrs(val.Block.ID)
								if staticText := attrs[av.NodeAttrViewStaticText+"-"+destAv.ID]; staticText != "" {
									val.Block.Content = staticText
									val.Block.Subtype = "s" // 静态锚文本
								}
	...
	for _, item := range collection.GetItems() {
		for _, value := range item.GetValues() {
			...
			if av.KeyTypeBlock == destKey.Type {
				for _, content := range value.Rollup.Contents {
					if nil != content.Block {
						content.Block.Subtype = "d"
						attrs := GetBlockAttrs(content.Block.ID)
						if staticText := attrs[av.NodeAttrViewStaticText+"-"+destAv.ID]; staticText != "" {
							content.Block.Content = staticText
							content.Block.Subtype = "s"
						}
					}
				}
			}
