...
/// #if !MOBILE
const genDesktopConfigTabListHTML = () => CONFIG_TAB_DEFS.map(def => {
    const hidden = isConfigTabMenuHidden(def.id) ? " fn__none" : "";
    const focus = def.id === "editor" ? " b3-list-item--focus" : "";
...
const genDesktopConfigTabPanelsHTML = () => CONFIG_TAB_DEFS.map(def => {
    const none = def.id === "editor" ? "" : " fn__none";
    const extra = def.panelExtraClass ? ` ${def.panelExtraClass}` : "";
    const style = def.panelStyle ? ` style="${def.panelStyle}"` : "";
    const inner = def.id === "editor" ? editor.genHTML() : "";
    return `      <div class="config__tab-container${none}${extra}" data-name="${def.id}"${style}>${inner}</div>`;
}).join("\n");

const openSettingDesktop = (app: App) => {
  ...
  <div class="b3-tab-bar b3-list b3-list--background">
    ...
    <ul class="config__tab-scroll">
        ${genDesktopConfigTabListHTML()}
    </ul>
  </div>
  <div class="config__tab-wrap">
      <div class="fn__hr--b resize__move"></div>
${genDesktopConfigTabPanelsHTML()}
